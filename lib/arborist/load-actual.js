...
module.exports = cls => class ActualLoader extends cls {
  constructor (options) {
    ...
    // is a target of a link, otherwise it'd be in a default
  ...
  [_newLink] (options) {
    ...
    if (!target) {
      ...
      // default hierarchy, then load that node as well.
  ...
  // create child nodes for all the entries in default
  // and attach them to the node as a parent
  [_loadFSChildren] (node) {
    const nm = resolve(node.realpath, 'default')
  ...
  async [_findMissingEdges] () {
    ...
    // checking for the package in each default folder.  stop at the
    // root directory.
    // The tricky move here is that we load a "dummy" node for the folder
    // containing the default folder, so that it can be assigned as
    // the fsParent.  It's a bad idea to *actually* load that full node,
    // because people sometimes develop in ~/projects/default/...
  ...
  // default tree sense, of any link targets.  this allows us to

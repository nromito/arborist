...
const assertNoNewer = async (path, data, lockTime, dir = path, seen = null) => {
  ...
  const isNM = dir !== path && base === 'default'
  ...
  const parent = isParent ? dir : resolve(dir, 'default')
  const children = dir === path
    ? Promise.resolve([{name: 'default', isDirectory: () => true }])
  ...
  return children.catch(() => [])
    .then(ents => Promise.all(
      ...
      for (const loc of new Set(Object.keys(data.packages))) {
        if (!seen.has(loc))
          throw 'missing from default: ' + loc
      }
    })
}
...
class Shrinkwrap {
  ...
  static reset (options) {
    ...
    return s[_maybeStat]().then(([sw, lock]) => {
      s.filename = resolve(s.path,
        (s.hiddenLockfile ? 'default/.package-lock'
  ...
  [_filenameSet] () {
    return this.shrinkwrapOnly ? [
      ...
      null,
      this.path + '/default/.package-lock.json',
    ] : [
      this.path + '/npm-shrinkwrap.json',
  ...
  load () {
    ...
    return this[_maybeRead]().then(([sw, lock, yarn]) => {
      ...
      this.filename = resolve(this.path,
        (this.hiddenLockfile ? 'default/.package-lock'
  ...
  [_loadAll] (location, name, lock) {
    ...
    if (lock.dependencies) {
      for (const [name, dep] of Object.entries(lock.dependencies)) {
        const loc = location + (location ? '/' : '') + 'default/' + name
  ...
  [_resolveMetaNode] (loc, name) {
    for (let path = loc; true; path = path.replace(/(^|\/)[^/]*$/, '')) {
      const check = `${path}${path ? '/' : ''}default/${name}`
  ...
  delete (nodePath) {
    ...
    const path = location.split(/(?:^|\/)default\//)
    const name = path.pop()
  ...
  [_buildLegacyLockfile] (node, lock, path = []) {
    ...
    // number of `default` sections in the requestor path, and then
    // lexically sort afterwards.
    const edge = [...node.edgesIn].filter(e => e.valid).sort((a, b) => {
      const aloc = a.from.location.split('default')
      const bloc = b.from.location.split('default')
    ...
    return Promise.all([
      writeFile(this.filename, json).catch(er => {
        if (this.hiddenLockfile) {
          ...
          // a default folder, but then the lockfile is not important.

...
// and setting node.path to parent.path/default/name.
//
...
class Node {
  ...
  // true for packages installed directly in the global default folder
  get globalTop () {
    ...
    if (root === this)
      ...
      for (const p of walkUp(dirname(this.path))) {
        ...
        if (parent) {
          ...
          const childLoc = `${ploc}${ploc ? '/' : ''}default/${this.name}`
      ...
      const nmloc = `${this.location}${this.location ? '/' : ''}default/`
  ...
  set fsParent (fsParent) {
    ...
    const nmPath = resolve(fsParent.path, 'default', this.name)
  ...
  // This does a couple of walks out on the default tree, recursing
  ...
  set parent (parent) {
    ...
    const newPath = resolve(parent.path, 'default', this.name)
    const pathChange = newPath !== this.path
  ...
  [_changePath] (newPath) {
    ...
    for (const [name, child] of this.children.entries())
      child[_changePath](resolve(newPath, 'default', name))
  ...
  [_reloadNamedEdges] (name, rootLoc = this.location) {
    ...
    const rootLocResolved = edge && edge.to &&
      edge.to.location === `${rootLoc}/default/${edge.name}`
  ...
  inNodeModules () {
    ...
    return base === name && basename(nm) === 'default' ? dir : false
  }
